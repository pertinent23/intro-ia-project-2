...
class PacmanAgent(Agent):
    ...
    def get_action(self, state: GameState):
        ...
        # On ajoute une dimension "batch" de
        # taille 1 car le réseau attend [Batch, Features]
        # x passe de [9] à [1, 9]
        x = x.unsqueeze(0)
        # 3. Prédiction du réseau
        with torch.no_grad():
            # Sortie brute du réseau (scores)
            logits = self.model(x)
        # 4. Sélection de la meilleure action LÉGALE
        # Le réseau nous donne 5 scores
        # On ne garde que ceux des actions possibles.
