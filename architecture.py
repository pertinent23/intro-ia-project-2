...
class PacmanNetwork(nn.Module):
    ...
    def __init__(self, input_size, num_actions=5):
        ...
        self.net = nn.Sequential(
            ...
            # On projette les features vers 512 neurones
            nn.Linear(input_size, 512),
            # Fonction d'activation Leaky ReLU
            # Elle introduit de la non-linéarité
            # pour apprendre des fonctions complexes
            nn.LeakyReLU(),
            nn.Dropout(0.4),
            # Couche cachée 1 -> Couche cachée 2
            # On réduit la dimensionnalité pour forcer
            # le réseau à synthétiser l'info
            nn.Linear(512, 256),
            nn.LeakyReLU(),
            nn.Dropout(0.2),
